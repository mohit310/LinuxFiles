...
if [ -z "$JDK" -a -s "$HOME/.IdeaIC2017.3/config/idea.jdk" ]; then
  USER_JRE=`"$CAT" $HOME/.IdeaIC2017.3/config/idea.jdk`
  if [ ! -d "$USER_JRE" ]; then
...
elif [ -r "$HOME/.IdeaIC2017.3/config/idea$BITS.vmoptions" ]; then
  # user-overridden
  VM_OPTIONS_FILE="$HOME/.IdeaIC2017.3/config/idea$BITS.vmoptions"
...
if [ "$IS_EAP" = "true" ]; then
  ...
  if [ -r "$AGENT_PATH" ]; then
    AGENT="-agentpath:$AGENT_PATH=probe_disable=*,disablealloc,disabletracing,onlylocal,disableexceptiontelemetry,delay=10000,sessionname=IdeaIC2017.3"
  fi
fi
...
"$JAVA_BIN" \
  ...
  -Didea.paths.selector=IdeaIC2017.3 \
